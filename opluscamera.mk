...
OPLUS_CAMERA_PATH := device/oneplus/macanc-camera

# NOTE: the device-agnostic camera Soong modules (libapsfixup, libQnn*_gallery_system_ext,
# CameraThemedIcon) are defined once, globally, in device/oneplus/macan-camera and shared —
# they are byte-identical across macan/macanc and the vendor libAlgoProcess blob references
# "libapsfixup" by that exact name. Only the camera-app libs (Android.mk), configs, sepolicy,
# props and proprietary blobs below are macanc-specific.
...
PRODUCT_PACKAGES += \
    ...
    libNativeWinBuffExchange_camera_app_lib_macanc \
    libHeifEncoderWrapper_camera_app_lib_macanc \
...
# SEpolicy
include device/oneplus/macanc-camera/sepolicy/SEPolicy.mk

# Inherit from camera-vendor.mk
$(call inherit-product-if-exists, vendor/oneplus/macanc-camera/macanc-camera-vendor.mk)
